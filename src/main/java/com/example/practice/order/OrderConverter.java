...
import com.example.practice.domain.order.OrderItem;
import com.example.practice.order.controller.MemberDto;
import com.example.practice.domain.order.item.Car;
import com.example.practice.domain.order.item.Food;
import com.example.practice.domain.order.item.Furniture;
	...
	private List<OrderItem> convertOrderItems(OrderDto orderDto) {
		return orderDto.getOrderItemDtos().stream()
			.map(orderItemDto -> {
				...
				Item item = convertItem(orderItemDto.getItemDto());
